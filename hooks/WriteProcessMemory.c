 ...
 *
!*/
D_SEC( D ) BOOL WINAPI WriteProcessMemory_Hook( _In_ HANDLE hProcess, LPVOID Address, LPVOID Buffer, SIZE_T Length, SIZE_T *Written )
{
	API			Api;
	SYS			Sys;
	UNICODE_STRING		Uni;
	...
	if ( NT_SUCCESS( ( Nst = Api.NtOpenSection( &Sec, SECTION_MAP_EXECUTE | SECTION_MAP_READ, &Att ) ) ) ) {
		if ( NT_SUCCESS( ( Nst = Api.NtMapViewOfSection( Sec, NtCurrentProcess(), &Ntm, 0, 0, NULL, &Len, ViewUnmap, 0, PAGE_READONLY ) ) ) ) {
			Sys.NtWriteVirtualMemory = PeGetFuncEat( Ntm, H_API_NTWRITEVIRTUALMEMORY );
			Nst = Sys.NtWriteVirtualMemory( hProcess, Address, Buffer, Length, Written );
