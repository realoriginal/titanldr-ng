...
D_SEC( D ) NTSTATUS NTAPI NtMapViewOfSection_Hook( HANDLE Section, HANDLE Process, PVOID* Base, ULONG_PTR Zero, SIZE_T CommitSize, PLARGE_INTEGER Offset, PSIZE_T ViewSize, SECTION_INHERIT SectionInherit, ULONG Type, ULONG Protect )
{
	...
	if ( NT_SUCCESS( ( Nst = Api.NtOpenSection( &Sec, SECTION_MAP_EXECUTE | SECTION_MAP_READ, &Att ) ) ) ) {
		if ( NT_SUCCESS( ( Nst = Api.NtMapViewOfSection( Sec, NtCurrentProcess(), &Ntm, 0, 0, NULL, &Len, ViewUnmap, 0, PAGE_READONLY ) ) ) ) {
			Sys.NtMapViewOfSection = PeGetFuncEat( Ntm, H_API_NTMAPVIEWOFSECTION );
			Nst = Sys.NtMapViewOfSection( Section, Process, Base, Zero, CommitSize, Offset, ViewSize, SectionInherit, Type, Protect );
			Api.NtUnmapViewOfSection( NtCurrentProcess(), Ntm );
		};
		Api.NtClose( Sec );
	};
	Api.RtlSetLastWin32Error( Api.RtlNtStatusToDosError( Nst ) ); return Nst;
};
