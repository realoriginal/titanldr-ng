	...
	if ( NT_SUCCESS( Api.LdrLoadDll( NULL, 0, &Uni, &K32 ) ) ) {
		...
		if ( Asn ) 
		{
			...
			if ( NT_SUCCESS( Api.NtCreateEvent( &Ovl.hEvent, EVENT_ALL_ACCESS, NULL, NotificationEvent, FALSE ) ) ) {
				...
				Ret = Api.WriteFile( hFile, lpBuffer, nNumberOfBytesToWrite, lpNumberOfBytesWritten, &Ovl );
				...
				if ( Ret != TRUE ) {
					...
					if ( NtCurrentTeb()->LastErrorValue == ERROR_IO_PENDING ) {
						...
						/* Get the result. */
						Ret = Api.GetOverlappedResult( hFile, &Ovl, lpNumberOfBytesWritten, FALSE );
					};
				};
		...
		{
			/* Execute WriteFile as normal */
			Ret = Api.WriteFile( hFile, lpBuffer, nNumberOfBytesToWrite, lpNumberOfBytesWritten, lpOverlapped );
		};
